    ...
    {   if(current_t>=tmin)
        {
            ...
            // compute beer and powder terms, from new_light_read
            // vec3 beer = exp( -1.0f / new_light_read.rgb) * alpha_squared * t_color.a;
            vec3 beer = exp( -1.0f / new_light_read.rgb) * alpha_squared * t_color.a;
            vec3 powder = vec3(1.0f) - beer;

            // apply the lighting scaling, using the new terms
            // old method : new_read.rgb *= new_light_read.rgb;
            // new_read.rgb *= beer * powder * new_light_read.rgb;
